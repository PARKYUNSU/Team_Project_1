...
def scrape_flight_data(driver, collect_url, date):
    ...
    for result_element in result_elements:
        flight_dict = {
            "airline": result_element.find_element_by_css_selector(
                "div.airline > b.name"
            ).text,
            "date": date[0:4] + "-" + date[4:6] + "-" + date[6:8],
            "departure_time": result_element.find_elements_by_class_name(
                "route_time__-2Z1T"
            )[0].text,
            "arrival_time": result_element.find_elements_by_class_name(
                "route_time__-2Z1T"
            )[1].text,
            "departure_place": result_element.find_elements_by_class_name(
                "route_code__3WUFO"
            )[0].text,
            "arrival_place": result_element.find_elements_by_class_name(
                "route_code__3WUFO"
            )[1].text,
            "flight_time": sum(
                int(t) * 60**i
                for i, t in enumerate(
                    result_element.find_element_by_class_name("route_info__1RhUH")
                    .text[:-1]
                    .split("시간 ")[::-1]
                )
            ),
            "seat": result_element.find_element_by_class_name(
                "domestic_type__30RSq"
            ).text,
            "price": int(
                result_element.find_element_by_class_name(
                    "domestic_num__2roTW"
                ).text.replace(",", "")
            ),
        }
...
df.to_csv("flight_data_08.csv", index=False, encoding="utf-8-sig")
